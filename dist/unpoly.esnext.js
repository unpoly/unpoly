...
up.util = (function () {
    ...
    */
    ...
    assignPolyfill = function (target, ...sources) {
    ...
    */
    ...
    presence = function (value, tester = isPresent) {
        if (tester(value)) {
            return value;
        }
        else {
    ...
    */
    ...
    only = function (object, ...properties) {
    ...
    except = function (object, ...properties) {
        var filtered, i, len, property;
    ...
    */
    ...
    evalOption = function (value, ...args) {
        if (isFunction(value)) {
            return value(...args);
        }
        else {
...
up.Record = class Record {
    ...
    attributes(source = this) {
        return u.only(source, ...this.fields());
    }
    [`${u.copy.key}`]() {
        return this.variant();
    }
...
@class up.Request
*/
ref = up.Request = class Request extends up.Record {
    /***
    ...
    */
    constructor(options) {
        ...
        this.extractHashFromUrl = this.extractHashFromUrl.bind(this);
        this.transferParamsToUrl = this.transferParamsToUrl.bind(this);
        this.transferSearchToParams = this.transferSearchToParams.bind(this);
        ...
        this.isCrossDomain = this.isCrossDomain.bind(this);
    ...
    send() {
        ...
        return new Promise((resolve, reject) => {
            ...
            xhrHeaders = u.copy(this.headers);
            xhrUrl = this.url;
            xhrParams = u.copy(this.params);
            xhrMethod = up.proxy.wrapMethod(this.method, xhrParams);
            xhrPayload = null;
            ...
            if (this.target) {
                xhrHeaders[pc.targetHeader] = this.target;
            }
            if (this.failTarget) {
                xhrHeaders[pc.failTargetHeader] = this.failTarget;
            }
            ...
            resolveWithResponse = () => {
                var response;
                response = this.buildResponse(xhr);
    ...
    */
    addAll(raw) {
        if (u.isMissing(raw)) {
            ...
            return this.entries.push(...raw.entries);
        }
        else if (u.isArray(raw)) {
            // internal use for copying
            return this.entries.push(...raw);
        }
    ...
    addAllFromFormData(formData) {
        return u.eachIterator(formData.entries(), (value) => {
...
up.Tether = class Tether {
    constructor(options) {
        ...
        [this.position, this.align] = options.position.split('-');
        if (this.align) {
    ...
    setBoundsOffset(left, top) {
        ...
        this.offsetTop = top;
        return e.setStyle(this.root, { left, top });
    }
};
...
u = up.util;
e = up.element;
up.CompilePass = class CompilePass {
    ...
    compile() {
        return up.log.group("Compiling fragment %o", this.root, () => {
            var compiler, i, len, ref, results;
            ref = this.compilers;
            results = [];
            for (i = 0, len = ref.length; i < len; i++) {
                compiler = ref[i];
                results.push(this.runCompiler(compiler));
            }
            return results;
        });
    }
    runCompiler(compiler) {
        ...
        return up.log.group((!compiler.isDefault ? "Compiling '%s' on %d element(s)" : void 0), compiler.selector, matches.length, () => {
...
up.Selector = (function () {
    ...
    class Selector {
        constructor(selector1, filterFn) {
        ...
        subtree(root) {
            ...
            matches.push(...this.descendants(root));
            return matches;
        }
        closest(root) {
...
u = up.util;
e = up.element;
up.HtmlParser = class HtmlParser {
    constructor(html) {
        this.html = html;
    ...
    wrapNoscriptInHtml() {
        ...
        return this.html = this.html.replace(noscriptPattern, (match, content) => {
            this.didWrapNoscript = true;
...
u = up.util;
e = up.element;
up.EventListener = class EventListener {
    constructor(element1, eventName1, selector1, callback1, options = {}) {
    ...
    */
    static parseArgs(args) {
        ...
        selector = args[0];
        return { elements, eventNames, selector, callback };
    }
    static bind(args, options) {
    ...
    static key(eventName, selector, callback) {
    ...
    static unbindNonDefault(element) {
    ...
    makeRoomForAnotherKey() {
        ...
        if (max && storeKeys.length >= max) {
            ...
            u.each(storeKeys, (key) => {
                var entry, timestamp;
                entry = this.store.get(key); // we don't need to call cacheKey here
...
var u;
u = up.util;
up.ExtractCascade = class ExtractCascade {
    constructor(selectorOrElement, options) {
        ...
        this.plans = u.map(this.candidates, (candidate, i) => {
            var planOptions, ref;
            planOptions = u.copy(this.options);
            if (i > 0) {
                // If we're using a fallback (any candidate that's not the first),
                // the original transition might no longer be appropriate.
                planOptions.transition = (ref = up.fragment.config.fallbackTransition) != null ? ref : this.options.transition;
            }
...
u = up.util;
e = up.element;
up.ExtractPlan = class ExtractPlan {
    ...
    findNew() {
        return u.each(this.steps, (step) => {
            ...
            return step.newElement = this.response.first(step.selector);
        });
    }
    oldExists() {
        this.findOld();
    ...
    resolveNesting() {
        ...
        compressed = u.filter(compressed, (candidateStep, candidateIndex) => {
            return u.every(compressed, (rivalStep, rivalIndex) => {
    ...
    parseSteps(originalSelector) {
        ...
        return u.each(disjunction, (expression, i) => {
            ...
            doReveal = i === 0 ? this.reveal : false;
...
ref = up.store.Session = class Session extends up.store.Memory {
    constructor(rootKey) {
        super();
        this.clear = this.clear.bind(this);
        this.set = this.set.bind(this);
...
up.MotionController = class MotionController {
    ...
    startFunction(cluster, startMotion, memory = {}) {
        ...
        if (memory.trackMotion === false) {
            ...
            return this.finish(cluster).then(() => {
                var promise;
                promise = this.whileForwardingFinishEvent(cluster, mutedAnimator);
                promise = promise.then(() => {
                    return this.unmarkCluster(cluster);
                });
    ...
    */
    ...
    expandFinishRequest(elements) {
        if (elements) {
            return u.flatMap(elements, (el) => {
                return e.list(e.closest(el, this.selector), e.all(el, this.selector));
            });
        }
        else {
    ...
    whileForwardingFinishEvent(cluster, fn) {
        ...
        doForward = (event) => {
            if (!event.forwarded) {
                return u.each(cluster, (element) => {
                    if (element !== event.target && this.isActive(element)) {
                        return this.emitFinishEvent(element, {
                            forwarded: true
                        });
                    }
                });
...
up.CssTransition = class CssTransition {
    ...
    startFallbackTimer() {
        var timingTolerance;
        timingTolerance = 100;
        return this.fallbackTimer = u.timer(this.totalDuration + timingTolerance, () => {
            return this.finish();
        });
    }
    stopFallbackTimer() {
...
@class up.Response
*/
ref = up.Response = class Response extends up.Record {
    /***
    ...
    */
    constructor(options) {
        ...
        */
        ...
        @experimental
        */
        this.isFatalError = this.isFatalError.bind(this);
        /***
...
u = up.util;
e = up.element;
up.FollowVariant = class FollowVariant {
    constructor(selector, options) {
    ...
    registerEvents() {
        up.on('click', this.fullSelector(), (...args) => {
            return u.muteRejection(this.onClick(...args));
        });
        return up.on('mousedown', this.fullSelector('[up-instant]'), (...args) => {
            return u.muteRejection(this.onMousedown(...args));
        });
    }
    followLink(link, options = {}) {
        ...
        promise = promise.then(() => {
            ...
            return this.followNow(link, options);
        });
        if (!options.preload) {
...
@internal
*/
up.DivertibleChain = class DivertibleChain {
    constructor() {
        this.reset = this.reset.bind(this);
...
up.event = (function () {
    ...
    */
    ...
    logEmission = function (eventName, eventProps) {
        ...
        if (u.isString(message)) {
            if (u.isPresent(eventProps)) {
                return up.puts(`${message} (%s (%o))`, ...messageArgs, eventName, eventProps);
            }
            else {
                return up.puts(`${message} (%s)`, ...messageArgs, eventName);
            }
        }
    ...
    nobodyPrevents = function (...args) {
        var event;
        event = emit(...args);
        return !event.defaultPrevented;
    };
    whenEmitted = function (...args) {
...
up.log = (function () {
    ...
    */
    ...
    callConsole = function (stream, ...args) {
        return console[stream](...args);
    };
    ...
    sprintf = function (message, ...args) {
        return sprintfWithFormattedArgs(u.identity, message, ...args);
    };
    sprintfWithFormattedArgs = function (formatter, message, ...args) {
        var i;
...
up.syntax = (function () {
    ...
    */
    ...
    registerJQueryMacro = function (...args) {
        var macro;
...
up.viewport = (function () {
    ...
    */
    ...
    allSelector = function () {
        ...
        return [rootSelector(), ...config.viewports].join(',');
    };
    ...
    fixedElements = function (root = document) {
        var queryParts;
...
up.fragment = (function () {
    ...
    */
    ...
    createPlaceholder = function (selector, container = document.body) {
    ...
    */
    ...
    ajax = function (...args) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            return makeRequest(...args).then(pickResponseText, reject);
        });
    };
    isIdle = function () {
...
up.form = (function () {
    ...
    */
    ...
    observe = function (elements, ...args) {
...
up.modal = (function () {
    ...
    */
    ...
    markAsAnimating = function (isAnimating = true) {
    ...
    flavorDefault = function (key, flavorName = state.flavor) {
        var value;
        if (flavorName) {
...
up.feedback = (function () {
    ...
    */
    ...
    buildCurrentUrlSet = function () {
        ...
        return new up.UrlSet(urls, { normalizeUrl });
    };
