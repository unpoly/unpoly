...
up.history = (function() {
  ...
  */
  function trackCurrentLocation(originEvent) {
    ...
    console.debug("[trackCurrentLocation] nextPrevious: %o, current: %o", nextPreviousLocation, location)
    ...
    if (nextPreviousLocation !== location) {
      ...
      let change = { location, state, base, hash, previousLocation, previousBase, previousHash, log: `New location is ${location}` }
      if (up.framework.booted) {
        if (originEvent) up.log.putsEvent(originEvent)
        let locationChangedEvent = up.event.build('up:location:changed', change)
        // TODO: Move this to unpoly-migrate and also deprecate { reason }
        up.migrate?.renamedProperty?.(locationChangedEvent, 'url', 'location')
