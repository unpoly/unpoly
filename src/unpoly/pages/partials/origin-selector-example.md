...
We can simplify the `[up-target]` by referencing the followed link as `:origin`:
...
When a link is clicked, `:origin` is replaced with a target [derived](/target-derivation) from the link element.
