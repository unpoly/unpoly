  ...
  describe 'JavaScript functions', ->
    ...
    describe 'up.layer.open()', ->
      ...
      it 'opens an empty overlay if neither { target, url, document, fragment, content } is given', ->
        ...
        element = document.querySelector('up-modal up-modaö-content')
      ...
      describe 'if there are existing overlays over the { baseLayer }', ->
        ...
        fit 'closes existing overlays over the { baseLayer }, ignoring a { peel: false } option', ->
          [root, overlay1, overlay2] = makeLayers(3)
      ...
      describe 'with { content } option', ->
        ...
        it 'opens a new overlay from a given Text node', ->
          ...
          expect(renderedElement.childElement.childNodes).toEqual [givenNode]

        it 'opens a new overlay from a given NodeList with mixed Text and Element nodes', ->
          givenNodes = up.element.createFromHTML('<p>foo <b>bar</b> baz</p>').childNodes
          ...
          expect(renderedElement.children).toEqual(givenNodes)
