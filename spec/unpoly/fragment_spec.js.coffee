  ...
  #              expect(@lastRequest().requestHeaders['X-Up-Target']).toEqual('.outer:after, .inner')
    ...
    describe 'up.reload()', ->
      ...
      describe 'last modification time', ->
        ...
        it 'sends an If-Modified-Since header with the timestamp of an ancestor if the fragment itself has no [up-time] attribute', asyncSpec (next) ->
          parent = fixture('.parent[up-time="1445412480"]')
          element = e.affix(parent, '.element[up-source="/source"]')
        ...
        it "sends no If-Modified-Since header if the fragment has [up-time=false], indicating that its earlier response had no Last-Modified header", asyncSpec (next) ->
          element = fixture('.element[up-source="/source"][up-time=false]')
