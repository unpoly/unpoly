up.magic = (function() {
  ...
  function newBehavior(options) {
    ...
    behavior.install = function() {
      ...
      _.each(registered, function(description) {
  ...
  $(document).on('ready', function() {
    app.element.install($(document.body));
    app.selector.install();
    app.timeout.install();
    app.interval.install();
    app.once.install();
    page.element.install($(document.body));
  ...
  $(document).on('page:hibernate', function() {
    page.selector.uninstall();
    page.timeout.uninstall();
    page.interval.uninstall();
  });

  $(document).on('page:ready', function($page) {
    page.selector.install();
    page.timeout.install();
    page.interval.install();
    app.element.install($page);
    page.element.install($page);
  });
