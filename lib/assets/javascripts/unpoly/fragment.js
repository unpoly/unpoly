...
up.fragment = (function() {
  ...
  */
  ...
  @internal
  */
  const timeOf = element => e.closestAttr(element, 'up-time') || '0';
  ...
  */
  ...
  const isDestroying = element => !!e.closest(element, '.up-destroying');
  ...
  */
  ...
  @stable
  */
  const visit = (url, options) => navigate(u.merge({ url }, options));

  const successKey = key => u.unprefixCamelCase(key, 'fail');
  ...
  */
  ...
  var parseSelector = function(selector, element, options = {}) {
    ...
    let expandedTargets = up.fragment.expandTargets(selector, u.merge(options, {layer: layers[0]}));
