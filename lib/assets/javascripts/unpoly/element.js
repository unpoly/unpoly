...
//      fn(args...)
  ...
  */
  const metaContent = function(name) {
    ...
    return __guard__(first(selector), x => x.getAttribute('content'));
  };
...
//    u.presence(value)
  ...
  */
  const jsonAttr = function(element, attribute) {
    ...
    let json;
    if (json = __guard__(typeof element.getAttribute === 'function' ? element.getAttribute(attribute) : undefined, x => x.trim())) {
  ...
  const closestAttr = (element, attr) => __guard__(closest(element, '[' + attr + ']'), x => x.getAttribute(attr));
