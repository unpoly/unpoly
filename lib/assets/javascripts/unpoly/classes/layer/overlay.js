...
up.Layer.Overlay = class Overlay extends up.Layer {
  ...
  */
  ...
  createElement(parentElement) {
    ...
    const elementAttrs = u.compactObject({ align: this.align, position: this.position, size: this.size, class: this.class, nesting: this.nesting });
  ...
  tryCloseForLocation(urlPattern, closeFn) {
    ...
    if (urlPattern && (location = this.location) && (resolution = urlPattern.recognize(location))) {
      ...
      const closeValue = u.merge(resolution, { location });
  ...
  */
  destroyElements(options) {
    ...
    const destroyOptions = u.merge(options, { animation, onFinished, log: false });
