...
Navigation intent
------------------
- Not every update is a user navigation
...
You will see some major changes, but most of them map to 0.x features.
...
Renamed modules:    E.g. up.proxy.config will return up.network.config
Renamed selectors:  E.g. <a up-close> will translate to <a up-dismiss>
...
- Async functions no longer wait for animations to finish
- New HTML structure for overlays
- Tooltips are gone
- [up-layer=modal] used to mean "update an existing modal", it now means "open a new modal".
...
Learning: A lot of links simply replace the main content area.
We often have a Rails helper like `content_link()` to set that default target.
...
Instead of using `[up-main]` you may also configure main targets globally:
    ...
    up.fragment.config.targets.push('.layout')
...
In Unpoly 1.0 these helpers (and their accompanying macros) are no longer needed.
You can now configure Unpoly to handle standard `<a href>` links without any `[up-...]` attributes.
As a Rails user this means you can just use `link_to` without extra option.
...
To except external links and forms, skip URL containing "://":

    up.link.config.followSelectors.push('a:not(href*="://")')
    up.form.config.submitSelectors.push('form:not(action*="://")')

Your links now no longer need an `[up-target]` or `[up-follow]` attribute:

    <a href="/backend">...</a>

Note that individual links or forms may still opt out of following by setting [up-follow=false]:
...
All your links now preload on `mouseover` and activate on `mousedown`:
...
Instead of giving each link an `[up-target]` attribute, mark the default targets with `[up-main]` in your layout:

    <div class="content" up-main>
      ...
    </div>

All your links will now update the `.content` fragment:
...
I'm actually not a bug fan of animating every fragment update.
...
The root page, modals and popups are now consolidated into a single term "layer".
...
| `{ layer: 'current' }   `| The current layer (Unpoly 1.0 default)                          |
| `{ layer: 'any' }       `| Any layer, preferring the current layer (Unpoly 0.x default)    |
| `{ layer: 'closest' }   `| The current layer or any ancestor, preferring closer layers     |
| `{ layer: 'parent' }    `| The layer that opened the current layer                         |
| `{ layer: 'ancestor' }  `| Any ancestor layer of the current layer                         |
| `{ layer: 'child' }     `| The child layer of the current layer                            |
| `{ layer: 'descendant' }`| Any descendant of the current layer                             |
| `{ layer: 'root' }      `| The root layer                                                  |
| `{ layer: 'overlay' }   `| Any overlay                                                     |
| `{ layer: 'origin' }    `| The layer of the element that triggered the current action      |
| `{ layer: 'Element' }   `| The given element's layer                                       |

When updating fragments you have two additional options:

| `{ layer: 'swap' }`      | Replace the front layer with a new layer                        |
| `{ layer: 'new' }`       | Open a new layer                                                |
...
- `[up-modal]` is deprecated => use [up-layer="modal"]
- `[up-popup]` is deprecated => use [up-layer="popup"]
...
End when a location is reached
------------------------------
...
The overlay will close when a URL like "/companies/123" is reached.

Placeholders in the URL pattern become the overlay's *acceptance value*.
...
Or to reload select options and selecting the new ID:
...
We now use custom elements. If you have customized your modals and popup with CSS, this is the largest breaking change for you.
...
New pseudo-classes for targets
==============================
...
- up:link:preload *new
- up:form:submit
- up:form:validate *new

Event handlers may prevent these events to cancel the fragment.

[NEW] Unpoly 1.x lets event listeners manipulate options for the `render()` call:
...
Loading errors and maintenance pages
====================================
...
Learning: We sometimes have multiple self-contained components on the same page. E.g. Deskbot cards.
...
Fragment updates don't always anymore.
...
| { revealPadding }  |  Pixels between element and viewport edge | up.viewport.config.revealPadding  | 0
| { revealTop }      |  Whether to align                         | up.viewport.config.revealTop      | false
| { revealMax }      |  How much of a high element to reveal     | up.viewport.config.revealMax      | 0.5 * window.innerHeight
| { revealSnap }     |  When to snap to the top edge             | up.viewport.config.revealSnap     | 200
| { scrollBehavior } |  auto/smooth                              | auto (no animation)               |
| { scrollSpeed }    |  Acceleration of smooth scrolling         | up.viewport.config.scrollSpeed    | 1


Reduce hash viewport jumps
--------------------------
...
Handlers like `[up-on-accepted]` work with CSP:
