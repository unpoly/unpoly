...
up.request() is now up.fetch()

The up.proxy package is now up.request. Conforms to pattern:
...
You can fine-tune this if you want:
    ...
    up.request.config.preloadMaxLoadTime: 750
    up.request.config.preloadSampleSize: 7 // initial page load is first sample
