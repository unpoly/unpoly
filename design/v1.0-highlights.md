...
- Enable multi-layered interactions
...
Parallel to promises.

There is up.layer.ask() that returns a promise for the acceptance value:

    try {
      let user = await up.layer.open({ url: '/users/new' })
      console.log("New user is " + user)
    } catch (e) {
      console.log("User creation was canceled")
    }
...
If your code *does* need to wait:
    ...
    up.destroy(element,, {
      onFinished: () => console.log("Overlay has finished closing animation")
    })
...
- Faster resolution, use onFinished if you need to wait
