...
- Update in current layer => Do nothing
- Any other layer => Use [up-layer]
...
Layers can be stacked inifinitely
=================================
...
In Unpoly 0.x it was very hard to communicate the "result" of an overlay interaction back to the parent layer. Overlay content always needed to know which element in a parent layer to target.
...
Prefer Demo un unpoly-demo-layers
...
The overlay will close when a URL like "/users/123" is reached.
...
Positive and negative close intent
==================================
...
There is also `up.layer.ask()`` that returns a promise for the acceptance value:
...
You can now style this in your CSS:
...
More complex:
    ...
    <a href="/likes" authorize>My likes</a>

    up.on('up:link:follow', 'a[authorize]', async function(event) {
      if (!isSignedIn()) {
        event.preventDefault()
        await up.layer.ask('/session/new', { acceptEvent: 'session:created' })
...
[up-instant] for your own components
====================================
