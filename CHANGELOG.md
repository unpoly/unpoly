...
- ⚠️ A field inside a `<fieldset disabled>` is no longer submitted or validated, but is still reported by [watchers](/up.watch) and [`[up-switch]`](/switching-form-state). Submission follows the browser, which omits it; watching reads the field's own disabled state, which an ancestor `<fieldset>` does not set.
...
- The contract for a [custom form field](/custom-form-fields#contract) is now stated in terms of what Unpoly reads, rather than requiring `{ name, value, disabled }` properties. A `value` getter is the only thing a custom control must provide: `name` and `disabled` are read from the property if there is one, and from the attribute otherwise. This relaxes the old requirement rather than replacing it.
- ⚠️ When Unpoly disables a custom control that exposes no `{ disabled }` property, it now sets the control's `[disabled]` attribute instead of assigning the property. Such a control no longer reports `element.disabled === true` while disabled, and a `click` listener registered with `up.on()` will no longer be suppressed for it.
